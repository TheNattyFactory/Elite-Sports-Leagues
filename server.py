...
class H(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if p=="/elite/enter":
            ...
            try:
                local=None
                if ext and str(ext.get("external_user_id") or "").strip():
                    local=c.execute("SELECT id,username,role FROM users WHERE id=?",(str(ext["external_user_id"]),)).fetchone()

                # First link: only auto-link an existing EBL account with the same username.
                # We do not create a password-bearing EBL account from SSO and do not overwrite another identity.
                if not local:
